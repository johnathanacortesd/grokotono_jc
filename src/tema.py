"""Agrupa subtemas similares en un tema_AI más general."""
...
TEMA_FAMILIES: list[tuple[frozenset[str], str]] = [
    ...
    (frozenset({"foro", "foros", "conversatorio", "campus"}), "Foros y eventos académicos"),
    ...
    (frozenset({"graduacion", "grados", "egresados"}), "Graduaciones"),
    (frozenset({"inversion", "inversiones", "presupuesto", "recursos"}), "Inversión y recursos"),
    (frozenset({"convocatoria", "convocatorias"}), "Convocatorias"),
    ...
    (frozenset({"suicidio", "suicidios", "jovenes"}), "Informes de salud pública"),
...
ACTION_HEADS = {
    ...
    "firma", "inauguracion", "encuentro", "evento", "gestion",
}
    ...
    if words and fold_text(words[0]) in ACTION_HEADS and len(words) >= 3:
        ...
        if len(rest) >= 2:
            ...
            if ocr_fold(broadened) != ocr_fold(phrase) and len(broadened.split()) >= TEMA_MIN_WORDS:
                return broadened
        if len(rest) == 1:
            return _clip_tema(f"{rest[0]} y gestión")
